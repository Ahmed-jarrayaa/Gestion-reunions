   ...
   public function index()
{
    ...
    // Charger la table Participants
    $participantsTable = \Cake\ORM\TableRegistry::getTableLocator()->get('Participants');

    // Récupérer le terme de recherche (GET)
    $search = $this->request->getQuery('search');

    // Requête de base : réunions de l'utilisateur
    $query = $participantsTable->find()
        ->where(['id_utilisateur' => $userId])
        ->contain(['Reunions']);

    // 🔎 Appliquer le filtre si "search" est présent
    if (!empty($search)) {
        $query->matching('Reunions', function ($q) use ($search) {
            return $q->where([
                'OR' => [
    ...
    $this->set(compact('participants', 'user', 'search'));
}
    ...
    {
        $user = $this->request->getAttribute('identity'); // Renvoie l'utilisateur connecté
